go0:-
  reconsult('http://www.binnetcorp.com/download/Jinni/netjinni/q8.pro'),
  go.

go00:-
  new_reader('http://www.binnetcorp.com/download/Jinni/netjinni/q8.pro',R),
  new_writer('temp.pro',W),
  discharge(R,W).
...
go11:-
  lazy_findall(X,member(X,[1,2,3,4]),LXs),
  lazy_element_of(LXs,A),
  lazy_element_of(LXs,B),
  println(LXs),
  println(pair(A,B)),
  fail.
go11:-
...
producer(Name,TC):-for(I,1,10),put(TC,from(Name,I)),println(put(Name,I)).

consumer(Name,TC):-for(_,1,10),collect(TC,the(X)),println(to(Name,X)).

go12:-
  hub(0,TC),
  bg(consumer(one,TC),_),
  bg(consumer(two,TC),_),
  bg(producer(three,TC),_),
  bg(producer(four,TC),_),
  println(main_here),
  %stop(TC),
  println(main_end).
...
bug:-
  ...
  bg(put(H,a),_),
  bg(put(H,b),_),
  bg(and(collect(H,R),println(R)),_),
  bg(and(collect(H,RR),println(RR)),_).
